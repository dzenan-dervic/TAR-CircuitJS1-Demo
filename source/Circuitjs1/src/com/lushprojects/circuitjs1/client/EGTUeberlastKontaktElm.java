/*
    TAR-Dervic EGT: Überlastrelais-Hilfskontakt 95–96 / 97–98, Dump 483
*/

package com.lushprojects.circuitjs1.client;

/**
 * Getrennter Öffner (95–96) oder Schließer (97–98), gekoppelt über Bezeichnung.
 * FLAG_NC = Öffner.
 */
class EGTUeberlastKontaktElm extends ChipElm implements EGTDesignatable {
    ...
    public EGTUeberlastKontaktElm(int xx, int yy) {
	super(xx, yy);
    ...
    public EGTUeberlastKontaktElm(int xa, int ya, int xb, int yb, int f,
				  StringTokenizer st) {
    ...
    boolean tripped() {
	EGTUeberlastLink.refresh(sim);
	return EGTUeberlastLink.isTripped(designation);
    }

    boolean contactClosed() {
	return isNC() ? !tripped() : tripped();
    }

    String getChipName() {
	return isNC() ? "EGT-Überlastrelais-Öffner"
		      : "EGT-Überlastrelais-Schließer";
    }
	...
	EGTStyle.stampPotentialFreeContact(sim, this, nodes[0], nodes[1],
		contactClosed(), EGTUeberlastLink.R_ON);
    }
	...
	return super.dump() + " " + sizeX + " " + sizeY + " "
		+ CustomLogicModel.escape(designation)
		+ EGTNote.dumpSuffix(note);
    }
    ...
    int getDumpType() { return 483; }
    String getXmlDumpType() { return "EGTUeberlastKontakt"; }

    void getInfo(String arr[]) {
	arr[0] = getChipName() + " (" + designation + ")";
	arr[1] = contactClosed() ? "geschlossen" : "offen";
	arr[2] = tripped() ? "ausgelöst" : "bereit";
	arr[3] = "I = " + getCurrentText(switchCurrent);
    }

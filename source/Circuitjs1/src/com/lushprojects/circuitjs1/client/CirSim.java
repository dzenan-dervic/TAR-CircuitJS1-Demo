...
public class CirSim implements NativePreviewHandler {
    ...
    /** Iframe von egt-hmi-tor.html / egt-hmi-pumpe.html — Menü-Geber auch ohne Query. */
    native boolean parentIsEgtHmi() /*-{
	try {
	    if (!$wnd.parent || $wnd.parent === $wnd)
		return false;
	    var p = $wnd.parent.location.pathname || "";
	    return p.indexOf("egt-hmi-tor") >= 0 || p.indexOf("egt-hmi-pumpe") >= 0;
	} catch (e) {
	    return false;
	}
    }-*/;
    ...
    /** HMI-Geber/Ausgang nur im Splitscreen-Menü (Tor/Pumpe). */
    boolean egtHmi;
	...
	try {
	    ...
	    egtHmi = qp.getBooleanValue("egtHmi", false) || parentIsEgtHmi();
	...
	undoManager.scheduleRecoveryPrompt();
    }

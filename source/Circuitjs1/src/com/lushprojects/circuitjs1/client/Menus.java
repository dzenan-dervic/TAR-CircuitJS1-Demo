...
public class Menus {
    ...
    CheckboxMenuItem egtLivePulseCheckItem;
    ...
    CirSim sim;
    MenuBar menuBar;
    /** Splitscreen: zweite Leiste UND/ODER/FUP direkt unter Datei/Zeichnen. */
    EGTFupBar egtFupMenuBar;
    CheckboxMenuItem[] egtLeiterWahlItems;
    CheckboxMenuItem egtLeitungEinfachItem;
    CheckboxMenuItem egtLeitungFlexibelItem;
    MenuBar fileMenuBar;
	...
	m.addItem(egtLivePulseCheckItem = new CheckboxMenuItem(Locale.LS("EGT Wire Follow Voltage Red")));
	egtLivePulseCheckItem.setState(true);
	...
	// EGT symbols (TAR-Dervic) — sits between Tools and Circuits
	MenuBar egtMenuBar = new MenuBar(true);
	egtMenuBar.setAutoOpen(true);
	getClassCheckItem(Locale.LS("EGT-Leitung"), "EGTLeitungElm");
	egtMenuBar.addItem(Locale.LS("EGT-Leitung"), composeEgtLeitungSubmenu());
	makeClassCheckItems(egtMenuBar, new String[] {
		...
		"EGT-Serienschalter (rastend)", "EGTSerienschalterElm",
		"EGT-Wechselschalter (rastend)", "EGTWechselschalterElm",
		"EGT-Kreuzschalter (rastend)", "EGTKreuzschalterElm",
		"EGT-Taster (tastend)", "EGTTasterElm",
		"EGT-Leuchte", "EGTLeuchteElm",
		"EGT-Meldeleuchte", "EGTMeldeleuchteElm",
		"EGT-Klingel", "EGTKlingelElm",
		"EGT-Türöffner", "EGTTueroeffnerElm",
		"EGT-Leistungsschütz (gekoppelt)", "EGTLeistungsschuetzElm",
		"EGT-Hilfsschütz (gekoppelt)", "EGTHilfsschuetzElm",
		"EGT-Hilfsschalterblock", "EGTHilfsschalterblockElm",
		"EGT-Drehstrommotor (Asynchron)", "EGTDrehstrommotorElm",
		"EGT-Gleichstrommotor", "EGTGleichstrommotorElm",
		"EGT-Kondensatormotor (vereinfacht)", "EGTKondensatormotorKompaktElm",
		"EGT-Kondensatormotor (Klemmbrett)", "EGTKondensatormotorElm",
		"EGT-Schütz-Spule", "EGTSchuetzSpuleElm",
		"EGT-Schütz-Leistungskontakte", "EGTSchuetzLeistungskontakteElm",
		"EGT-Schütz-Hilfskontakt", "EGTSchuetzHilfskontaktElm",
		"EGT-Stromstoßschalter (zusammen)", "EGTStromstossschalterElm",
		"EGT-Stromstoß-Spule", "EGTStromstossSpuleElm",
		"EGT-Stromstoß-Kontakt", "EGTStromstossKontaktElm",
		"EGT-Zeitrelais-Spule (Ein/Aus/Kombiniert/Blinkend)", "EGTZeitrelaisSpuleElm",
		"EGT-Zeitrelais-Kontakt (Ein/Aus/Kombiniert/Blinkend)", "EGTZeitrelaisKontaktElm",
		"EGT-Treppenlichtautomat", "EGTTreppenlichtElm",
		"EGT-Thermisches Überlastrelais", "EGTUeberlastrelaisElm",
		"EGT-Überlastrelais (Heizleiter)", "EGTUeberlastHeizleiterElm",
		"EGT-Überlastrelais-Kontakt", "EGTUeberlastKontaktElm",
		"EGT-Motorschutzschalter (3-polig)", "EGTMotorschutzschalterElm",
		"EGT-Sicherung", "EGTSicherungElm",
		"EGT-Bewegungsmelder (IR)", "EGTBewegungsmelderElm",
		"EGT-Mini-SPS (LOGO)", "EGTMiniSpsElm",
	});
	if (sim.egtHmi)
	    makeClassCheckItems(egtMenuBar, new String[] {
		    "EGT-HMI-Geber (Klemme I)", "EGTHmiGeberElm",
		    "EGT-HMI-Ausgang (Klemme Q)", "EGTHmiLastElm",
	    });
	makeClassCheckItems(egtMenuBar, new String[] {
		"EGT-FUP-Eingang (I)", "EGTFupEingangElm",
		"EGT-FUP-Ausgang (Q)", "EGTFupAusgangElm",
		"RS/SR-Speicher (FUP)", "RSSpeicherElm",
		"FUP-Einschaltverzögerung (TON)", "EGTFupTonElm",
	...
	if (sim.egtHmi)
	    composeEgtFupMenuBar();
    }

    /** Nur Splitscreen: Symbolleiste für Schüler direkt unter Datei/Zeichnen. */
    void composeEgtFupMenuBar() {
	egtFupMenuBar = new EGTFupBar();
    }
	...
	makeClassCheckItems(chipMenuBar, new String[] {
		...
		"Add RS/SR-Speicher (FUP)", "RSSpeicherElm",
    ...
    MenuBar composeEgtLeitungSubmenu() {
	MenuBar bar = new MenuBar(true);
	egtLeitungEinfachItem = new CheckboxMenuItem(
		Locale.LS("Einfach (L/Z)"));
	egtLeitungEinfachItem.setScheduledCommand(new Command() {
	    public void execute() {
		EGTLeitungElm.setPendingFlexible(false);
		CirSim.theApp.commands.menuPerformed("main", "EGTLeitungElm");
	    }
	});
	egtLeitungFlexibelItem = new CheckboxMenuItem(
		Locale.LS("Flexibel (Knick)"));
	egtLeitungFlexibelItem.setScheduledCommand(new Command() {
	    public void execute() {
		EGTLeitungElm.setPendingFlexible(true);
		CirSim.theApp.commands.menuPerformed("main", "EGTLeitungElm");
	    }
	});
	bar.addItem(egtLeitungEinfachItem);
	bar.addItem(egtLeitungFlexibelItem);
	bar.addSeparator();
	egtLeiterWahlItems = new CheckboxMenuItem[EGTLeitungElm.ROLE_NAMES.length];
	int i;
	for (i = 0; i < EGTLeitungElm.ROLE_MENU.length; i++) {
	    final int art = i;
	    CheckboxMenuItem mi = new CheckboxMenuItem(
		    Locale.LS(EGTLeitungElm.ROLE_MENU[i]));
	    mi.setScheduledCommand(new Command() {
		public void execute() {
		    EGTLeitungElm.setPendingLeiterArt(art);
		    CirSim.theApp.commands.menuPerformed("main",
			    "EGTLeitungElm");
		}
	    });
	    egtLeiterWahlItems[i] = mi;
	    bar.addItem(mi);
	}
	refreshEgtLeiterWahl();
	return bar;
    }

    void refreshEgtLeiterWahl() {
	if (egtLeitungEinfachItem != null)
	    egtLeitungEinfachItem.setState(!EGTLeitungElm.pendingFlexible);
	if (egtLeitungFlexibelItem != null)
	    egtLeitungFlexibelItem.setState(EGTLeitungElm.pendingFlexible);
	if (egtLeiterWahlItems == null)
	    return;
	int art = EGTLeitungElm.safeRole(EGTLeitungElm.pendingLeiterArt);
	int i;
	for (i = 0; i < egtLeiterWahlItems.length; i++)
	    if (egtLeiterWahlItems[i] != null)
		egtLeiterWahlItems[i].setState(i == art);
    }

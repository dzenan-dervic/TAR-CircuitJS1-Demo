...
public abstract class CircuitElm implements Editable {
    ...
    /** Geglättetes |I| für Strompunkte: hält AC-Nulldurchgänge, blendet bei echt I=0 aus. */
    double dotActivity;
...
//	    return shortFormat.format(Math.round(v));
    ...
    double updateDotCount(double cur, double cc) {
	 ...
	 if (!app.simIsRunning())
	    return cc;
	// Hüllkurve |I|: kurze AC-Nullen behalten Punkte; nach Schalter-AUS ausblenden
	double a = Math.abs(cur);
	dotActivity = Math.max(a, dotActivity * 0.78);
	if (dotActivity < 1e-8)
	    return 0;

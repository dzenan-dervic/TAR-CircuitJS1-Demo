...
public class UIManager {
    ...
    static final int EGTFUPBARHEIGHT = 36;

    int egtFupBarHeight() {
	if (hideMenu || app == null || !app.egtHmi || menus == null || menus.egtFupMenuBar == null)
	    return 0;
	return EGTFUPBARHEIGHT;
    }
	...
	menus.egtLivePulseCheckItem.setCommand(
		new Command() { public void execute(){
		    setOptionInStorage("egtLivePulse", menus.egtLivePulseCheckItem.getState());
		}
	});
	menus.egtLivePulseCheckItem.setState(getOptionFromStorage("egtLivePulse", true));
	...
	if (!hideMenu && app.egtHmi && menus.egtFupMenuBar != null)
	    layoutPanel.addNorth(menus.egtFupMenuBar, EGTFUPBARHEIGHT);
...
*/
    ...
    public void setCanvasSize(){
    	...
    	height=height- (hideMenu ? 0 : MENUBARHEIGHT) - egtFupBarHeight();
	...
	if (subcircuitBar != null) {
	    int barTop = (hideMenu ? 0 : MENUBARHEIGHT) + egtFupBarHeight();
	    if (menus.toolbarCheckItem.getState())
	...
	// CTM zurücksetzen vor Clear (HiDPI); sonst Ghost-Trails bei Chip-save/restore.
	cvcontext.setTransform(1, 0, 0, 1, 0, 0);
        g.fillRect(0, 0, (int) (canvasWidth * devicePixelRatio() + 0.5),
		    (int) (canvasHeight * devicePixelRatio() + 0.5));
	...
	app.scopeManager.drawHoverScope(g, canvasWidth, canvasHeight);
	...
	if (mouse.dragElm != null)
	    ...
	    String mode = mouseModeStr;
	    String label = app.classToLabelMap.get(mode);
	    if ("EGTLeitungElm".equals(mode))
		label = EGTLeitungElm.drawModeLabel();
	    toolbar.setModeLabel(Locale.LS("Mode: ") + label);
	    app.menus.refreshEgtLeiterWahl();
	}

...
public class MouseManager implements MouseDownHandler, MouseMoveHandler, MouseUpHandler,
 ...
 MouseOutHandler, MouseWheelHandler {
    ...
    public EGTFupEingangElm heldFupEingang;
	...
	if (mouseElm instanceof EGTMiniSpsElm) {
	    if (((EGTMiniSpsElm) mouseElm).toggleAt(x, y)) {
		sim.needAnalyze();
		return true;
	    }
	    return false;
	}
	if (mouseElm instanceof EGTBewegungsmelderElm) {
	    if (((EGTBewegungsmelderElm) mouseElm).triggerAt(x, y)) {
		sim.needAnalyze();
		return true;
	    }
	    return false;
	}
	if (mouseElm instanceof EGTFupEingangElm) {
	    EGTFupEingangElm ie = (EGTFupEingangElm) mouseElm;
	    if (!ie.hitBody(x, y))
		return false;
	    ie.clickDown();
	    if (ie.momentary)
    ...
    public void mouseDragged(MouseMoveEvent e) {
    	...
    	case MODE_SELECT:
    		if (mouseElm == null)
    			...
    			changed = true;
    		    } else if (mouseElm instanceof EGTLeitungElm
    			    && ((EGTLeitungElm) mouseElm).allowsKnickDrag()) {
    			tempMouseMode = MODE_DRAG_REROUTE;
    			((EGTLeitungElm) mouseElm).rerouteVia(snapGrid(gx), snapGrid(gy));
    			changed = true;
    		    } else {
  ...
  //  	console("dx="+dx+"dragGridx="+dragGridX);
    ...
    void dragPost(int x, int y, boolean all) {
    	if (draggingPost == -1) {
    	    int h = mouseElm.getHandleGrabbedClose(x, y, POSTGRABSQ, 0);
    	    if (h >= 0)
    		draggingPost = h;
    	    else
    		draggingPost =
    	...
    	if (all) {
    	    ...
    	    if (mouseElm instanceof EGTLeitungElm
    		    && ((EGTLeitungElm) mouseElm).allowsKnickDrag()
    		    && draggingPost > 0
    		    && draggingPost < mouseElm.getPostCount() - 1) {
    		((EGTLeitungElm) mouseElm).rerouteVia(x, y);
    	    } else {
    		mouseElm.movePoint(draggingPost, dx, dy);
    		if (sim.sim.routedWireMap != null) {
    		    ArrayList<SimulationManager.RoutedWireConnection> conns =
    			    sim.sim.routedWireMap.get(mouseElm);
    		    if (conns != null) {
    			for (SimulationManager.RoutedWireConnection conn : conns) {
    			    if (conn.elmPost == draggingPost
    				    && conn.wire.getPost(conn.wirePost).z == 0)
    				conn.wire.movePoint(conn.wirePost, dx, dy);
    			}
    		    }
    		}
...
//	    // left mouse
	...
	if (tempMouseMode == MODE_SELECT && mouseElm!=null && !ui.isReadOnly() &&
		...
		!anySelectedButMouse()) {
	    tempMouseMode = MODE_DRAG_POST;
	    draggingPost = mouseElm.lastHandleGrabbed;
	}
...
//
	...
	try {
	    ...
	    EGTDesignation.assignOnPlace(dragElm);
	} catch (Exception ex) {
	    sim.debugger();
	}
	...
	sim.menus.refreshEgtLeiterWahl();
    ...
    public void onMouseUp(MouseUpEvent e) {
    	...
    	if (heldFupEingang != null) {
    		heldFupEingang.clickUp();
    		heldFupEingang = null;
    		circuitChanged = true;
    	}
    	if (dragElm != null) {
